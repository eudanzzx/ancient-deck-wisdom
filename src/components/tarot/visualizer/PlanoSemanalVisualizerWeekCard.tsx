...
interface PlanoSemanalVisualizerWeekCardProps {
  week: PlanoWeek;
  index: number;
  onToggle: (index: number) => void;
}

const PlanoSemanalVisualizerWeekCard: React.FC<PlanoSemanalVisualizerWeekCardProps> = ({
  week,
  index,
  onToggle,
}) => {
    ...
    <Button
      ...
      className={`
        ...
        ${week.isPaid 
          ...
          : 'bg-gradient-to-br from-white via-slate-50 to-white hover:from-slate-50 hover:via-white hover:to-slate-50 border-slate-300 text-slate-700 shadow-lg shadow-slate-300/30 hover:border-blue-400/60 hover:shadow-blue-300/40'
        }
      `}
    >
      ...
      <div className="relative z-10 text-center space-y-1">
        <div className={`
          ...
          ${week.isPaid 
            ...
            : 'bg-gradient-to-r from-slate-700 via-blue-700 to-slate-700 bg-clip-text text-transparent group-hover:from-blue-600 group-hover:via-cyan-600 group-hover:to-blue-600'
          }
        `}>
          {week.week}ª
        </div>
      ...
      <div className="relative z-10 text-center space-y-1">
        <div className="flex items-center justify-center gap-1">
          <CalendarDays className={`h-3 w-3 ${week.isPaid ? 'text-white/80' : 'text-slate-500'}`} />
          <div className={`
...
export default PlanoSemanalVisualizerWeekCard;
